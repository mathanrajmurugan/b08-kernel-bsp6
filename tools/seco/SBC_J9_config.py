...
Select an available board configuration."""))
        ...
        self.txt_j9 = wx.StaticText(self, -1, "J9: ");
        ...
        self.txt_j9.SetLabelText("J9: %s" % self.__bsel.getJ9(item))
        ...
        self.sizer.Add(self.txt_j9, 0, wx.ALL, 5)
...
class TestPanel(wx.Panel):
    ...
    def OnRunWizard(self, bsel):
        ...
        wizard = wiz.Wizard(self, -1, "SBC J9 Expansion Connector Configurator", images.sbc_j9.GetBitmap())

	    # Board Selection
        page1 = BoardSelectPage(wizard, "Board Selection", self.__bsel, images.sbc_j9)
        wizard.FitToPage(page1)

        page2  = PortConfigPage(wizard, "Config Port - 1", 1, "./ports.conf", images.sbc_j9_p1)
        page3  = PortConfigPage(wizard, "Config Port - 2", 2, "./ports.conf", images.sbc_j9_p2)
        page4  = PortConfigPage(wizard, "Config Port - 3", 3, "./ports.conf", images.sbc_j9_p3)
        page5  = PortConfigPage(wizard, "Config Port - 4", 4, "./ports.conf", images.sbc_j9_p4)
        page6  = PortConfigPage(wizard, "Config Port - 5", 5, "./ports.conf", images.sbc_j9_p5)
        page7  = PortConfigPage(wizard, "Config Port - 6", 6, "./ports.conf", images.sbc_j9_p6)
        page8  = PortConfigPage(wizard, "Config Port - 7", 7, "./ports.conf", images.sbc_j9_p7)
        page9  = PortConfigPage(wizard, "Config Port - 8", 8, "./ports.conf", images.sbc_j9_p8)
        page10 = PortConfigPage(wizard, "Config Port - 9", 9, "./ports.conf", images.sbc_j9_p9)
        page11 = PortConfigPage(wizard, "Config Port - 10", 10, "./ports.conf", images.sbc_j9_p10)
        page12 = PortConfigPage(wizard, "Config Port - 11", 11, "./ports.conf", images.sbc_j9_p11)
        page13 = PortConfigPage(wizard, "Config Port - 12", 12, "./ports.conf", images.sbc_j9_p12)
        page14 = PortConfigPage(wizard, "Config Port - 13", 13, "./ports.conf", images.sbc_j9_p13)
        ...
        if wizard.RunWizard(page1):
            ...
            if ( os.path.isfile(dtsfile) == True ):
                ...
                if ( retCode == wx.ID_NO ):
                    ...
                    wx.MessageBox("Wizard not completed", "SBC J9 Config")
                    sys.exit ()
            ...
            if ( os.path.isfile(dtsfile) == True ):
                wx.MessageBox("Wizard completed successfully", "SBC J9 Config")
            else:
                wx.MessageBox("Wizard not completed, impossible to create dts file", "SBC J9 Config")
            
        else:
        
            wx.MessageBox("Wizard was cancelled", "SBC J9 Config")
...
if __name__ == "__main__":
    ...
    try:
        ...
        print 'SBC_J9.py -d <dts file output>'
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print 'SBC_J9.py -d <dts file output>'
